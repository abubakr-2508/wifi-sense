...
6. **Detection controls** — the sliders. This is the interactive moment: drag
   the **enter threshold** down and the state flips to MOTION more readily; the
   exit threshold auto-adjusts to keep the hysteresis gap. Say: *"You can set
   the sensitivity yourself — lower the threshold and it triggers on less. The
   gap between enter and exit is the hysteresis that stops it chattering."*
   Offer the examiner the mouse. Only the thresholds are live-adjustable;
   window length and top-K are fixed at launch because changing them resizes
   the buffers, and their effect is in the ablation study instead.

### Then switch to the Findings tab

Click **Findings** in the top bar. Six cards, each a real analysis figure:
the subcarrier-K result, the segment test that refutes respiration, the motion
ROC (AUC 0.94–1.0), the two-dataset discrimination, the phase negative, and the
MT7922 hardware negative. Walk through 1 → 6. This is where the demo and the
evidence become one screen — say *"everything here is generated by the analysis
scripts from real data; nothing is drawn by hand."*
