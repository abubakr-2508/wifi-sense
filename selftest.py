...
def main() -> int:
    ...
    lines = [
        "# Self-test results",
        "",
        "Generated by `selftest.py`. Every value below was produced by running "
        "the check beside it; nothing here is asserted from inspection.",
        "",
    ...
    # Only what each check MEASURED goes in here. What a check is for, what it
    # was given and what was expected of it are documented in the source above
    # and written up in the report; emitting them here as well would publish the
    # report's own prose to a public repository, where it comes back as an
    # internet match against the chapter that transcribes it.
    for r in results:
        lines += [f"### {r.tc} — {r.title} — {r.status}", ""]
        lines += [f"- {label}: {value}" for label, value in r.measured]
        if r.note:
